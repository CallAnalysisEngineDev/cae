...
public class SongDaoImpl implements ISongDao {
	...
	public DaoResult getSongForHomepageDao() {
		...
		String sql = "SELECT song_id,song_name,song_cover,(song_click/timestampdiff(hour,song_create_time,'"
				+ getNowTime() + "')) AS clickrate " + "FROM song " + "ORDER BY clickrate DESC " + "LIMIT "
				+ HOMEPAGE_RED_LIMIT;
		...
		sql = "SELECT song_id,song_name,song_cover " + "FROM song "
				+ "ORDER BY song_last_modify_time DESC,song_name ASC " + "LIMIT " + HOMEPAGE_NEWEST_LIMIT;
	...
	public DaoResult getAllSongDao(Condition condition, Song song) {
		...
		// 获取所有的歌曲
		String sql = "SELECT song_id,song_cover,song_owner,song_name " + "FROM song " + sqlWithParams.getWhere()
				+ sqlWithParams.getOrder() + "LIMIT " + condition.getPageStart() + "," + condition.getPageLimit();
		Object[] params = sqlWithParams.getParams();
		List<Song> theResult = template.query(sql, params,
				(rs, row) -> new Song.Builder().songId(rs.getString("song_id"))
						.songCover(IConstant.STATIC_PREFIX + rs.getString("song_cover"))
						.songOwner(rs.getString("song_owner")).songName(rs.getString("song_name")).build());
		return new DaoResult(true, theResult);
	}
	...
	public SqlWithParams getTheSqlForGetAll(Condition condition, Song song) {
		...
		if (isNotNull(condition.getOrderBy())) {
			...
			orderBuffer.append("ORDER BY " + condition.getOrderBy() + " " + (orderDir != null ? orderDir : "DESC")
					+ ",song_name ASC ");
		} else {
	...
	public DaoResult getSongCountDao(Condition condition, Song song) {
		...
		String sql = "SELECT COUNT(*) " + "FROM song " + sqlWithParams.getWhere();
		Object[] params = sqlWithParams.getParams();
	...
	public DaoResult getSongDao(Song song) {
		try {
			...
			Song theResult = template.queryForObject(sql, new Object[] { song.getSongId() },
					(rs, row) -> new Song.Builder().songName(rs.getString("song_name"))
	...
	public DaoResult saveSongDao(Song song) {
		try {
			...
			String sql = "INSERT INTO song(song_id,song_name,song_sell_time,song_owner,song_cover,song_create_time,song_click,song_last_modify_time,song_video)"
					+ "VALUES(?,?,?,?,?,?,?,?,?)";
			template.update(sql, song.getSongId(), song.getSongName(), song.getSongSellTime(), song.getSongOwner(),
					song.getSongCover(), getNowTime(), 0, getNowDate(), song.getSongVideo());
