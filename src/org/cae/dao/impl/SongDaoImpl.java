...
public class SongDaoImpl implements ISongDao {
	...
	public DaoResult getSongForHomepageDao() {
		...
		String sql = "SELECT song_id,song_name,song_cover,(song_click/timestampdiff(hour,song_create_time,'"
				...
				+ "')) AS clickrate "
				+ "FROM song "
				+ "ORDER BY clickrate DESC " + "LIMIT " + HOMEPAGE_RED_LIMIT;
		RowMapper<Map<String, Object>> rowMapper = new RowMapper<Map<String, Object>>() {
			...
			public Map<String, Object> mapRow(ResultSet rs, int row)
				...
				map.put("songCover",
						IConstant.STATIC_PREFIX + rs.getString("song_cover"));
				return map;
			}
	...
	public DaoResult getAllSongDao(Condition condition, Song song) {
		...
		String sql = "SELECT song_id,song_cover,song_owner,song_name "
				+ "FROM song " + sqlWithParams.getWhere()
				+ sqlWithParams.getOrder() + "LIMIT "
	...
	public SqlWithParams getTheSqlForGetAll(Condition condition, Song song) {
		...
		if (isNotNull(condition.getOrderBy())) {
			...
			orderBuffer.append("ORDER BY " + condition.getOrderBy() + " "
					+ (orderDir != null ? orderDir : "DESC")
					+ ",song_name ASC ");
		} else {
		...
		if (orderBuffer.length() > 0)
			return new SqlWithParams(whereBuffer.toString(),
					orderBuffer.toString(), params);
		else
	...
	public DaoResult getSongDao(Song song) {
		try {
			...
			Song theResult = template.queryForObject(sql,
					new Object[] { song.getSongId() }, new RowMapper<Song>() {
						@Override
						public Song mapRow(ResultSet rs, int row)
								throws SQLException {
							return new Song.Builder()
	...
	public DaoResult saveSongDao(Song song) {
		try {
			...
			String sql = "INSERT INTO song(song_id,song_name,song_sell_time,song_owner,song_cover,song_create_time,song_click,song_last_modify_time,song_video)"
					+ "VALUES(?,?,?,?,?,?,?,?,?)";
			template.update(sql, song.getSongId(), song.getSongName(),
					song.getSongSellTime(), song.getSongOwner(),
					song.getSongCover(), getNowTime(), 0, getNowDate(),
					song.getSongVideo());
